// ExpressionRecognition.cpp : Defines the entry point for the console application.
//
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	// creating a window
	HWND hwnd = CreateWindowEx(WS_EX_TOOLWINDOW, L"LISTBOX", L"Expression Recognition", 0, 0, 0, 0, 0, 0, 0, 0, 0); 
	HDC dc = GetDC(hwnd);
	...
	while (msg.message != WM_QUIT) {
		...
		if (FSDK_GrabFrame(cameraHandle, &imageHandle) == FSDKE_OK) { // grab the current frame from the camera
			...
			for (int i = 0; i < faceCount; i++) {
				...
				int res = FSDK_GetTrackerFacialAttribute(tracker, 0, IDs[i], "Expression", AttributeValues, sizeof(AttributeValues));
				...
				char str[1024];
				sprintf_s(str, sizeof(str)-1, "Smile: %d%%; Eyes open: %d%%", (int)(ConfidenceSmile*100), (int)(ConfidenceEyesOpen*100));
