...
const NSString * help_text = @"ExpressionRecognition is as much a developer tool as it is a conceptual proof of technology showcasing Luxand FaceSDK";
...
- (id)initWithScreen:(UIScreen *)newScreenForDisplay
{
    ...
    */
    ...
    if ((self = [super initWithNibName:nil bundle:nil])) {
        ...
        FSDK_SetTrackerMultipleParameters(_tracker, "DetectGender=false;DetectExpression=true", &errpos);
#if defined(DEBUG)
        if (errpos)
...
#if defined(DEBUG)
        if (errpos)
            NSLog(@"FSDK_SetTrackerMultipleParameters 3 returned errpos = %d", errpos);
#endif
...
{
    UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"ExpressionRecognition" message:(NSString *)help_text delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
...
- (void)processImageAsyncWith:(NSData *)args
{
    ...
    for (size_t i = 0; i < (size_t)count; ++i) {
        ...
        char values[1024];
        FSDK_GetTrackerFacialAttribute(_tracker, 0, IDs[i], "Expression", values, sizeof(values));
        ...
        int confidenceSmilePercent = confidenceSmile * 100;
        int confidenceEyesOpenPercent = confidenceEyesOpen * 100;
        NSString * attributes = [NSString stringWithFormat:@"Smile:%d%%\nEyes open:%d%%\n", confidenceSmilePercent, confidenceEyesOpenPercent];
