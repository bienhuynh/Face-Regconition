...
const NSString * help_text = @"AgeGenderRecognition is as much a developer tool as it is a conceptual proof of technology showcasing Luxand FaceSDK";
...
- (id)initWithScreen:(UIScreen *)newScreenForDisplay
{
    ...
    */
    ...
    if ((self = [super initWithNibName:nil bundle:nil])) {
        ...
        FSDK_SetTrackerMultipleParameters(_tracker, "DetectAge=true;DetectGender=true;DetectExpression=false", &errpos);
#if defined(DEBUG)
        if (errpos)
...
{
    UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"AgeGenderRecognition" message:(NSString *)help_text delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
...
- (void)processImageAsyncWith:(NSData *)args
{
    ...
    for (size_t i = 0; i < (size_t)count; ++i) {
        ...
        FSDK_GetTrackerFacialAttribute(_tracker, 0, IDs[i], "Age", values, sizeof(values));
        ...
        NSString * attributes = [NSString stringWithFormat:@"Age: %d,%@:%d%%\n", (int)confidenceAge,(confidenceMale>confidenceFemale)?@"Male":@"Female", MAX(confidenceMalePercent, confidenceFemalePercent)];
