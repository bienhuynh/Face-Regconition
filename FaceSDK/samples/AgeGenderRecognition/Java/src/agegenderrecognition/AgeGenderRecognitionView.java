 ...
 */
public class AgeGenderRecognitionView extends FrameView {
    public AgeGenderRecognitionView(SingleFrameApplication app) {
        super(app);
        ...
        try {
            int r = FSDK.ActivateLibrary("fhPKopJVjHpnsV6/aumzjOvApHU7gnFduuovOu1DRngQEEevUnlpLfSAhNIhLVjzPYYbhmrz36x9Xnn1AZ/8HOcgXtIaZxOxZzNXNwS3ezLiwFwZGhY9w3S+beKvuIC8DhosdzRYCSGx4H9hC1A+jAHqTPjBTr42nKiOZ+y3H70=");
            if (r != FSDK.FSDKE_OK){
        ...
        int err[] = new int[1];
        err[0] = 0;
        FSDK.SetTrackerMultipleParameters(tracker, "RecognizeFaces=false; DetectAge=true; DetectGender=true; HandleArbitraryRotations=false; DetermineFaceRotationAngle=false; InternalResizeWidth=100; FaceDetectionThreshold=5;", err);
                ...
                if (FSDKCam.GrabFrame(cameraHandle, imageHandle) == FSDK.FSDKE_OK){
                    ...
                    if (FSDK.SaveImageToAWTImage(imageHandle, awtImage, FSDK_IMAGEMODE.FSDK_IMAGE_COLOR_24BIT) == FSDK.FSDKE_OK){
                        ...
                        for (int i=0; i<faceCount[0]; ++i) {
                            ...
                            int res2 = FSDK.GetTrackerFacialAttribute(tracker, 0, IDs[i], "Age", AttributeValuesAge, 1024);
                            if (FSDK.FSDKE_OK == res && FSDK.FSDKE_OK == res2) { // draw
                                ...
                                String str = "Age: " + Integer.toString((int)(ConfidenceAge[0])) + ", ";
    ...
    private void initComponents() {
        ...
        javax.swing.ActionMap actionMap = org.jdesktop.application.Application.getInstance(agegenderrecognition.AgeGenderRecognitionApp.class).getContext().getActionMap(AgeGenderRecognitionView.class, this);
        jButton1.setAction(actionMap.get("buttonStart")); // NOI18N
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(agegenderrecognition.AgeGenderRecognitionApp.class).getContext().getResourceMap(AgeGenderRecognitionView.class);
