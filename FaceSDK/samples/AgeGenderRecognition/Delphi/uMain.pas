...
var
  ...
  AttributeValuesAge: array[0..1023] of char;
  ConfidenceMale, ConfidenceFemale, ConfidenceAge: single;
  s, s2: string;
  TextWidth: integer;
...
begin
  ...
  FSDK_SetTrackerMultipleParameters(tracker, 'RecognizeFaces=false; DetectAge=true; DetectGender=true; HandleArbitraryRotations=false; DetermineFaceRotationAngle=false; InternalResizeWidth=100; FaceDetectionThreshold=5;', @err);
  ...
  begin
    ...
    begin
      ...
      if ((0 = FSDK_GetTrackerFacialAttribute(tracker, 0, IDs[i], 'Gender', @AttributeValues, sizeof(AttributeValues))) and (0 = FSDK_GetTrackerFacialAttribute(tracker, 0, IDs[i], 'Age', @AttributeValuesAge, sizeof(AttributeValuesAge)))) then
      begin
        ConfidenceAge := 0;
        ConfidenceMale := 0;
        ConfidenceFemale := 0;
        FSDK_GetValueConfidence(AttributeValuesAge, 'Age', @ConfidenceAge);
        ...
        str(round(ConfidenceAge), s);
		s := 'Age: ' + s + ', ';

        if (ConfidenceMale > ConfidenceFemale) then
        begin
          str(round(100*ConfidenceMale), s2);
          s := s + 'Male, ' + s2 + '%';
        end
        else
        begin
          str(round(100*ConfidenceFemale), s2);
          s := s + 'Female, ' + s2 + '%';
        end;
